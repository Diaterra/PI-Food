...
return (
        ...
        //.map((element)=recipe_id> {return (
            ...
            <h2>{recipe_id.dishTypes?.map(element=>element)}</h2> 
              <p>{recipe_id.dish_summary}</p>  
            <h2>{recipe_id.health_score}</h2>
          <p>{recipe_id.instructions ? recipe_id.instructions : 'This recipe does not the instructions'}</p>   
          <ul> {recipe_id.diets?.map(e => <li>{e.name}</li>)}</ul>
          </div>
